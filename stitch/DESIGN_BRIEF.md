...
- BadgePing Mini turns the user's request into a directly usable web workflow. The first experience must be the actual web product behavior, not a marketing landing page or placeholder demo.
- - FR-001: Build a tiny single-page utility called BadgePing Mini. It should manage five status badges, let the user toggle pass/warn/fail states, persist to localStorage, and show an exportable JSON summary. Keep it static HTML/CSS/JS only.
...
- Five Operations - BadgePing Mini
- Five Editor - BadgePing Mini
- Insights - BadgePing Mini

## SCREEN_SPECS
SCREEN_SPEC_1:
- exact_screen_title: Five Operations - BadgePing Mini
- surface_id: SURF_FIVE_OPERATIONS
- unique_canvas_caption: Five Operations: summary metrics, primary list/board/table, filters, search, selected item preview, empty/loading/error states.
- purpose: Give the user the main operational view for inspecting, searching, filtering, and acting on Five data.
...
- data_entities: Five, ActivityEvent, Preference
...
SCREEN_SPEC_2:
- exact_screen_title: Five Editor - BadgePing Mini
- surface_id: SURF_FIVE_EDITOR
- unique_canvas_caption: Five Editor: form fields, required/optional indicators, validation messages, save/cancel controls, unsaved-state feedback.
- purpose: Let the user create, edit, validate, save, cancel, and recover Five changes.
...
- data_entities: Five, ValidationError
- visible_actions: ACT_SAVE_RECORD as form_submit, ACT_CANCEL_EDIT as secondary_button
- entry_exit_rules: SURF_FIVE_OPERATIONS -> Save returns to SURF_FIVE_OPERATIONS with persisted changes; cancel preserves existing data and closes the editor.
- design_guidance: Form layout must be clear and task-specific; do not invent payment, onboarding, or unrelated identity forms.

SCREEN_SPEC_3:
- exact_screen_title: Insights - BadgePing Mini
...
- purpose: Show useful summaries, trends, and status signals derived from Five data without becoming a separate analytics product.
- required_content: small metrics, recent activity, state distribution, actionable follow-up hints, empty/error state.
- data_entities: Five, ActivityEvent
- visible_actions: ACT_FILTER_INSIGHTS as context_menu, ACT_EXPORT_SUMMARY as secondary_button
- entry_exit_rules: SURF_FIVE_OPERATIONS -> No external BI/admin area; returns to SURF_FIVE_OPERATIONS.
...
## PRODUCT_SURFACES
1. SURF_FIVE_OPERATIONS - Five Operations
   Purpose: Give the user the main operational view for inspecting, searching, filtering, and acting on Five data.
   Data: Five, ActivityEvent, Preference
...
2. SURF_FIVE_EDITOR - Five Editor
   Purpose: Let the user create, edit, validate, save, cancel, and recover Five changes.
   Data: Five, ValidationError
   ...
   Entry/exit: SURF_FIVE_OPERATIONS -> Save returns to SURF_FIVE_OPERATIONS with persisted changes; cancel preserves existing data and closes the editor.
...
3. SURF_INSIGHTS - Insights
   ...
   Entry/exit: SURF_FIVE_OPERATIONS -> No external BI/admin area; returns to SURF_FIVE_OPERATIONS.
